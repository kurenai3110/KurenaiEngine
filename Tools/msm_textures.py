...
#
# 【花崗岩だけ天井に余裕が残っていたので0.5m相当(深さ8m)から上げた】3つのうち
# 上限60度で頭打ちになる割合は 岩1.8% / 石積21.9% / 乱石積20.4% で、岩だけ余地があった。
# 振った実測(岩の表面の局所コントラスト。参考写真は0.448):
#   深さ  8m 傾き中央29.3度 頭打ち 1.8%  岩0.338  島CV0.519  h0.2-0.3=0.2635
#   深さ 12m      40.1度      10.6%  岩0.361  島CV0.523
#   深さ 16m      48.3度      23.4%  岩0.377  島CV0.525   ← 採用
#   深さ 22m      57.0度      42.5%  岩0.394  島CV0.528  h0.2-0.3=0.2753
# 単調に良くなるが16mで止めるのは、23.4%が石積・乱石積と同じ頭打ちの領域だから。
# これを超えると勾配の半分近くが上限に張り付き、法線マップが「高さ場の勾配」ではなく
# 飽和した模様になる。22mの絵も見たが、得られるのは岩で+0.017・島CVで+0.003しかない
#
# 【スレート屋根には入れない(試して撤回)】高さ帯ごとの局所コントラストで、修道院の
# 屋根が支配する h0.6-0.7(SlateRoof 69%)が 0.242 対 参考写真 0.413 と最大の不足だった。
# 岩・石積みにはあるのに屋根に無かったので "slate" を足して振ったが、
#   深さ1.0m(傾き中央16.0度・頭打ち19.2%): 帯 0.242 → 0.248  … ほぼ効かない
#   深さ3.0m(傾き中央40.6度・頭打ち21.0%): 帯 0.242 → 0.279  … 数字は動く
# となり、3.0mでは**屋根が斑に破れたように見えた**。参考写真の大屋根を拡大すると
# 表面はむしろ滑らかで、あの帯のコントラストは屋根の質感ではなく
# 内陣の小尖塔・飛梁と、身廊/内陣/交差部の高さの違いが作っている。
# 数字は上がるが写真から遠ざかるので入れない。この帯を詰めるならジオメトリの側。
NORMAL_MAP_PARAMS = {
    "rock":    (16.00, 45.0),
    "masonry": (1.60, 15.0),
    "rubble":  (2.00, 12.0),
