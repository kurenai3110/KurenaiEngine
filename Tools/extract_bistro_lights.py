...
def format_scene(model, lights, camera, tally, counts, worst_overlap, blue_as_blue):
    ...
    w("# MegaLights の影レイは半透明(BLEND)を遮蔽物から外すが、笠・支柱・電球そのものは不透明なので")
    w("# 完全に遮る。器具の重心はその器具自身の内側にあるため、重心へ置くと全灯の影レイが自分の器具に")
    w("# 当たり、1灯も光らない(絵が暗いだけで例外もログも出ない)。")
    w("# なお下記の脱出率はガラスも遮蔽物として数えて測った値で、現在は保守的な見積もりになっている。")
