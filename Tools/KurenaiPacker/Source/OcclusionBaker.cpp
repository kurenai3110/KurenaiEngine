...
namespace KurenaiPacker
{
    namespace
    {
        ...
        //
        // 【SAHへ変える必要は無い】ベイク時間の99%はxatlasのUV展開が占めており、
        // レイキャストとBVH構築を合わせても0.2%に満たない(実測は docs/ImplementationDetail.md 22.6.6)。
        ...
        // 回るため、正しく実装していても比は下がる。これは正当な劣化なので、そこで警告を
        // 出すと「いつも鳴っている警告」になって用をなさない。狙っている不具合
        // (法線の向きに引きずられる)は浅い段から出るので、mip1〜3だけを見る
        // (両者がしきい値0.85を挟んで分かれることの実測は docs/ImplementationDetail.md 22.6.6)
...
void CSMain(uint3 id : SV_DispatchThreadID)
{
    ...
    // **縮小するほど暗くなる**(実測は docs/ImplementationDetail.md 22.6.6)。
