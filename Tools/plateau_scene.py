...
#
# 【10km にする根拠は大気遠近】以前はシーン対角(約47km)にしていた。「距離で切ると
# 遠景がそこで途切れる」という判断だったが、**そこまで見えていなかった**。
# 大気遠近の透過率は exp(-消散係数 x 距離) で、既定の 0.00040 だと
# 5km で 13.5% / 10km で 1.8% / 20km で 0.03% まで落ちる。
# 10km より遠いジオメトリは、載せてはいたが霞に埋もれて一度も見えていない。
#
# 対角(47380m)から 10000m へ下げた実測(RTX 4070 Ti / DX12 / Release / 1280x720 /
# 初期カメラ)。**地上の絵は152万画素で最大差1(量子化誤差)、実質1画素も変わらない**
# (同時に撮った空は雲が流れて52.6%の画素が変わっており、差の出る場所では出ている):
#
#   常駐モデル      767 -> 404
#   VRAM 使用/予算  11,086/9,246 MB -> 6,926/11,226 MB (予算超過が解消)
#   GPU待ち 中央値  4.12 ms -> 0.00 ms (35サンプルすべて 0.00)
#   最悪フレーム    60.50 ms -> 18.75 ms
#   RT高速化構造の構築  49.4 秒 -> 8.5 秒
#
# 根拠と測り方は docs/ImplementationDetail.md 60章。
#
# 【この値を上げるときは消散係数とセットで見る】霞が薄い設定にすると遠くまで見えるので、
# 10km で切ると本当に途切れる。[Sky] の消散係数を変えたらここも測り直すこと。
...
def main():
    ...
    # 大気遠近で見えなくなる距離で切る(定数のコメント参照)。
    # シーンが 10km より小さいなら切る意味が無いので、そのときは対角を使う
    streaming_distance = min(STREAMING_DISTANCE, diagonal)
    ...
    w('#')
    w('# 【10km にする根拠は大気遠近】以前はシーン対角(%.0fm)にしていた。' % diagonal)
    w('# 「距離で切ると遠景が途切れる」という判断だったが、そこまで見えていなかった。')
    w('# 透過率は exp(-消散係数 x 距離) で、既定の 0.00040 だと 5km で 13.5% /')
    w('# 10km で 1.8% / 20km で 0.03% まで落ちる。10km より遠いジオメトリは')
    w('# 載せてはいたが霞に埋もれて一度も見えていない。')
    w('#')
    w('# 対角から 10000m へ下げた実測(RTX 4070 Ti / DX12 / Release / 1280x720 /')
    w('# 初期カメラ)。地上の絵は152万画素で最大差1(量子化誤差)、実質1画素も変わらない:')
    w('#   常駐 767 -> 404 / VRAM 11,086 -> 6,926 MB(予算超過が解消) /')
    w('#   GPU待ち中央値 4.12 -> 0.00 ms / 最悪フレーム 60.50 -> 18.75 ms /')
    w('#   RT高速化構造の構築 49.4 -> 8.5 秒')
    w('#')
    w('# 【消散係数を変えたら測り直す】霞を薄くすると遠くまで見えるようになり、')
    w('# 10km で切ると本当に途切れる。根拠は docs/ImplementationDetail.md 60章。')
    w('#')
