...
DirectX 11 / DirectX 12 の両方に対応した自作ゲームエンジンです。**KurenaiEngineLibrary.dll**
(共通基盤)・**KurenaiEngine3D.dll**・**KurenaiEngine2D.dll** の3つのDLLに分かれてビルドされます。

- `Kurenai::KurenaiEngine3D` — Deferred Shading の完結型3Dレンダラー。専用モデルパッケージ
  (`.kmodel`)とシーンファイル(`.kscene`)を読んで描きます
- `Kurenai::KurenaiEngine2D` — スプライト・図形・テキスト描画を提供する軽量な2D API
- `Kurenai::RHI` / `Kurenai::Core` / `Kurenai::Assets`(まとめて「Library」)— RHI抽象化層・
  ウィンドウ・カメラ・モデル読み込みなどの低レベルAPI。独自の描画パイプラインを組みたい場合は
  こちらを直接使えます

**3Dと2Dは互いに依存しません。** 2Dのみのアプリは `KurenaiEngineLibrary.dll` +
`KurenaiEngine2D.dll` だけで動作し、`KurenaiEngine3D.dll` や Assets を同梱する必要はありません。

`KurenaiEngine3D` が備えるものは次のとおりです。**構成と仕様は
[docs/Architecture.html](docs/Architecture.html)** にあります。

| 分類 | 内容 |
|---|---|
| 描画 | Deferred Shading / HDRレンダリング / 半透明の専用フォワードパス / 内部レンダー解像度の実行時変更 |
| 影 | カスケードシャドウマップ(PCF / PCSS) / スクリーンスペースシャドウ / レイトレーシングシャドウ |
| 間接光 | IBL / DDGI / AO・間接光(SSAO / SSIL / RTAO・RTGI) / ベイク済み遮蔽マップ / Bent Normal |
| 反射 | スクリーンスペース反射(SSR) / 反射プローブ / レイトレーシング反射 / 平面反射 |
...
起動・操作・アセットの準備。書式の一覧・設計・既定値の根拠・経緯は下の4本が持ちます。
書き分けの定義は [CLAUDE.md](CLAUDE.md) の「ドキュメントの書き分け」にあります。
...
Tools/
  ...
  texdump_inspect.py                   `-dumptex` が書き出した中間レンダーターゲットの生値を
                                       ...
                                       連番から時間ノイズをタイル間・タイル内へ分けて測る `noise` もある。
                                       使う前に `selftest` を通すこと
...
```
  ...
  **半透明のマテリアルは遮蔽物として数えません**(glTFの`alphaMode=BLEND`のほか、OBJで`d`/`Tr`/`Tf`から半透明と判定したものも含みます) —— ガラスを遮蔽物として
  焼くと、ショーウィンドウの内側のような「窓が開いているだけの小空間」が完全密閉として
  扱われ、間接光が一切届かなくなるためです(外したメッシュ数はログに出ます)。
  アルファカットアウト(`alphaMode=MASK`)は従来どおり遮蔽物として数えます。
...
- **`--scale`は要りません。** 読み込み後の頂点はメートルです
- **`--specular-as-orm`が要ります。** ORMがFBXのSpecularColorスロットに入っています
...
実測と判断の根拠は [docs/ImplementationDetail.md](docs/ImplementationDetail.md) 43章。

#### 大規模な都市の確認用シーン(Project PLATEAU 東京都23区)

東京23区全域を、**建築物(LOD1/LOD2)・地形・道路・橋梁の4種**で構成したシーンです。
`.kmodel`を多数並べたときの読み込み・カリング・モデルLOD・ストリーミングの確認に使います。
...
スクリプトは ダウンロード → 5種の展開 → 共通原点の算出 → タイルのパック →
`.kscene`の生成と配置 → 出力フォルダへの配布 を順に行います。
`.kscene`は`Tools\plateau_scene.py`が機械生成します(767個の`[Model]`を手で書けないため)。

- **`--origin`が要ります。** 全タイルで同じ値(`-8096,0,-36118`)を引きます
- **`--scale`は要りません。** 入力はメートルです

実測と判断の根拠は [docs/ImplementationDetail.md](docs/ImplementationDetail.md) 45章、51章、55章。

##### メッシュレットLODの検証用シーン(dem / 地形)

PLATEAU の dem(地形)だけを配置した、メッシュレットLODの確認用シーンです。
...
- **`-dx12` が要ります。** メッシュレットLODはDX12のメッシュシェーダー経路で動きます
...
実測と判断の根拠は [docs/ImplementationDetail.md](docs/ImplementationDetail.md) 56章。

#### テクスチャ付きの近景シーン(Project PLATEAU 東京駅・丸の内 LOD2)

同じ配布物の**建築物LOD2**です。3次メッシュ`53394600`/`53394601`(東京駅・丸の内)の建築物と橋梁、
それを含む2次メッシュ`533946`の地形・道路をあわせた、実写テクスチャ付きの近景シーンです。
...
**`--origin`は23区シーンとまったく同じ値**にしてあります。

実測と判断の根拠は [docs/ImplementationDetail.md](docs/ImplementationDetail.md) 44章、45章。
...
`Tools\generate_meshlet_stage.py`は、ドラゴンを置くステージを生成します。
...
実測と判断の根拠は [docs/ImplementationDetail.md](docs/ImplementationDetail.md) 56.8節。

#### インスタンシングの確認用シーン

`Scenes\InstancingTest.kscene`は、同じ`.kmodel`を格子状に並べたインスタンシング確認用シーンです。
...
実測と判断の根拠は [docs/ImplementationDetail.md](docs/ImplementationDetail.md) 57章。
...
[docs/KurenaiEngine.html](docs/KurenaiEngine.html) の4.7節に、`.kscene`に書けるセクションとキーの一覧・
意味・既定値・範囲をまとめています。
READMEには、すぐ上のBistroExteriorの`.ini`の例のような最小限の記述例だけを残します。
...
MegaLightsの手法、蓄積ダンプの測定方法、および既定値の根拠は [docs/ImplementationDetail.md](docs/ImplementationDetail.md) 61章と64章。

| オプション | 意味 |
|---|---|
| `-scene <名前>` | 起動時に開くシーンを指定する。 |
| `-debugview <番号>` | デバッグ表示を番号で選ぶ。 |
| `-ddgiraster` | DDGIのレイ取得をラスタライズへ固定する。 |
| `-ddgithreshold <値>` | DDGIのプローブ分類しきい値を上書きする。 |
| `-megalights <番号>` | MegaLightsの手法を選ぶ。根拠は docs/ImplementationDetail.md 61.7j節。 |
| `-megalightsrays <本数>` | 参照実装が1灯あたりに撃つ影レイの本数を指定する。根拠は docs/ImplementationDetail.md 61.7k節。 |
| `-megalightssamples <M>` | 確率的サンプリングの候補数を指定する。 |
| `-megalightstemporal <0\|1>` | 時間再利用の有無を指定する。根拠は docs/ImplementationDetail.md 61.7b節。 |
| `-megalightstemporalmclamp <上限>` | 時間再利用の履歴上限を指定する。根拠は docs/ImplementationDetail.md 61.7b.1節。 |
| `-perfdump <パス>` / `-perfdumpframes <枚数>` | GPU区間計測の平均をCSVへ書き出す。根拠は docs/ImplementationDetail.md 61.7e.1節。 |
| `-dumptex <名前> <パス>` | 中間レンダーターゲットを線形の生値で書き出す。根拠は docs/ImplementationDetail.md 63章。 |
| `-dumptexmip <N>` / `-dumptexslice <N>` | 直前のダンプのミップ段・配列スライスを指定する。 |
| `-dumptexframes <N>` | 直前のダンプを連番で書き出す。 |
| `-dumptexstride <S>` | 連番捕獲のフレーム間隔を指定する。 |
| `-dumpframe <N>` | ダンプの書き出しフレームを指定する。 |
| `-exitafterdump` | ダンプ完了後に終了する。 |
| `-recreate <フレーム> <指示>` | 指定フレームでGPUリソースの再作成操作を実行する。 |
| `-taa <0\|1>` | TAAの有無を指定する。根拠は docs/ImplementationDetail.md 64.3節。 |
| `-megalightsdenoise <0\|1>` | MegaLightsデノイザの有無を指定する。 |
| `-megalightsdenoiseatrous <段数>` | a-trousフィルタの段数を指定する。根拠は docs/ImplementationDetail.md 61.7g.2節。 |
| `-megalightsdenoisesigma <値>` | デノイザの輝度エッジ停止の強さを指定する。根拠は docs/ImplementationDetail.md 61.7g.2節。 |
...
| `-megalightsspatialradius <ピクセル>` | 近傍探索の半径を指定する。 |
| `-megalightsspatialiters <回数>` | 空間再利用の反復回数を指定する。根拠は docs/ImplementationDetail.md 61.7g.3節。 |
| `-megalightsinitialvis <0\|1>` | 初期サンプルへの可視レイの有無を指定する。根拠は docs/ImplementationDetail.md 61.7f節。 |
| `-megalightsaccum <枚数>` | MegaLights出力を線形空間で指定枚数だけ蓄積する。根拠は docs/ImplementationDetail.md 61.5節。 |
| `-megalightsdump <パス>` | 蓄積したMegaLights出力を生データで書き出す。根拠は docs/ImplementationDetail.md 61.5節。 |
...
| 引数 | 意味 |
|---|---|
| `-emissivelights <0\|1>` | 自発光メッシュを光源として扱う。**既定は無効** |
| `-emissiveintensity <倍率>` | シーン全体の自発光の強度を変える。**既定は 1.0** |
| `-emissivelightscutoff <τ>` | 光源の影響半径を決める打ち切り照度を変える。**既定は 0.001** |
| `-emissivelightsmax <N>` | 採用するプロキシ数の上限を変える。**既定は 1024** |
| `-emissivelightsddgi <0\|1>` | DDGIへの自発光の加算を切り替える。**既定は 0** |
| `-meshlights <0\|1>` | 発光面を三角形のまま面積分する。**既定は無効**。下記の制限を読むこと |

#### `-meshlights` の制限

発光面を重心1点のプロキシへ潰すと、近くでは形が出ません。`-meshlights 1` は同じ発光体を
三角形の束のまま積分します。**ただし現在入っているのは参照実装(全三角形の総当たり)だけ**で、
次の制限があります。

- **DX12 + DXR が要ります。** MegaLights 経路でのみ効き、DX11・非DXR・MegaLights無効の
  ときは何も起きず、段階1のプロキシがそのまま光ります(そのぶん絵が変わります)
- **実シーンでは回りません。** 総当たりなので、三角形が数万枚あるシーンでは実用になりません。
  小さな専用シーン(`MeshLightQuad`)向けです
- 有効なフレームは段階1のプロキシを MegaLights が読み飛ばします。プロキシ自体は残るので、
  DDGI・反射プローブ・RT反射・半透明・平面反射はこれまでどおりプロキシで照らされます

導出と実測は [docs/ImplementationDetail.md](docs/ImplementationDetail.md) 62.9節にあります。

これらは起動後も「ライト」パネルの**「自発光の強度」スライダーの直下**から動かせます
(引数と同じ4つ + 現在のプロキシ数と送信中の灯数)。プロキシが1つも無いシーンでは
灰色になります。

小さな照明器具を明るくする逃げ道は2つあります。
...
単位変換を確かめるには、`Tools/generate_emissive_light_test.py` で同じモデルを使う2つのシーン
(自発光のプロキシ / 等価な手置きポイントライト)を生成し、床の明るさを比べます。

```
python Tools/generate_emissive_light_test.py
```

DDGIの二重計上(`-emissivelightsddgi`)を確かめるには、`Tools/generate_emissive_gi_test.py` で
4×4mの発光パネルを吊るシーンを生成し、`-debugview 26`(DDGIイラディアンスアトラス)を
つまみの0/1で比べます。

```
python Tools/generate_emissive_gi_test.py
```

検証手順と測れない理由は [docs/ImplementationDetail.md](docs/ImplementationDetail.md) 62.5節・62.11節にあります。
...
- **レンダリング** — シーンをどう照らすかに関わる品質設定を、次の8つの節に分けて調整します
  - **AO / 間接光** — ON/OFFと手法を「SSAO / SSIL (Visibility Bitmask) / レイトレーシング (RTAO/RTGI)」から選びます。どの手法も出力は共通(rgb=間接拡散光、a=遮蔽率)なので、切り替えても後段の扱いは変わりません
    - **SSAO** — 遮蔽率だけを求めます。最も軽量です。半径・強度に加えて**サンプル数**(1〜16、既定16)を指定できます。減らすほど遮蔽の推定は粗くなりますが、画素ごとにカーネルをランダム回転させたうえで後段のブラーで均します。実測と実装の根拠は [docs/ImplementationDetail.md](docs/ImplementationDetail.md) 22.6.8節にあります
  ...
  - **シャドウ** — 平行光(太陽)の影の手法を「なし / カスケードシャドウマップ (CSM) / レイトレーシング (RT)」から選びます。**半透明(`alphaMode=BLEND`)のマテリアルは、どの手法でも影を落としません**(ガラス越しに日が差す絵で、ガラスが窓の奥を真っ暗に潰してしまうためです)。アルファカットアウト(`alphaMode=MASK`)は影を落とします(CSMは切り抜きを反映し、RTは切り抜きを見ずに板ポリゴンのまま落とします)
  ...
  - **ソフトウェアラスタライザ**(既定OFF) — 三角形をコンピュートシェーダーで自前にラスタライズする**比較用の経路**です。ハードウェアがブラックボックスで行っている処理——頂点変換・背面カリング・スクリーン空間への投影・エッジ関数による被覆判定・深度テスト・透視補正補間——を明示的なコードとして持ち、G-Bufferと直接突き合わせられるようにするためのものです。**既存の描画経路には一切寄与せず**、結果は「デバッグ表示」の「SWラスタ」3種でのみ見ます。**DX12かつシェーダーモデル6.6・64bit整数のシェーダー演算(`Int64ShaderOps`)・bindless(`ResourceDescriptorHeap`)のすべてに対応したGPUでのみ操作でき**、それ以外の環境では理由を表示して常に無効です(基準実機のIntel UHD Graphics 620では動きません)
    - 深度と三角形IDを1つの64bit値へ詰めたvisibility bufferを使います。設計上の理由は [docs/Architecture.html](docs/Architecture.html) 40.4節にあります
    - 頂点とインデックスは、描画に使っているバッファそのものをbindlessで直接読みます(専用のコピーを持ちません)。中間の三角形レコードも持たず、三角形番号からジオメトリを引き直して再変換します
    - **巨大三角形のしきい値**(既定4096画素=64x64相当)を超えた三角形は、1スレッドで塗らず「1スレッドグループ=1三角形」の専用パスへ回します。しきい値は2つの経路が一致していることを確かめる対照実験にも使えます。実測は [docs/ImplementationDetail.md](docs/ImplementationDetail.md) 42.4節にあります
...
- **ポストプロセス** — トーンマップ(カーブはAgX / ACES / Reinhardから選択。既定はAgX。薄明視と出力ディザリングもここ)、TAA(ON/OFFと履歴ブレンド率・ジッター強度・シャープネス・静止時のちらつき抑制・履歴の棄却方法と許容幅)、ブルーム(強度・しきい値・ソフトニー)、自動露出(EV100の上下限・露出補正・測光値の上側クランプ・夜のロールオフ量・明順応/暗順応の速度・測光に使うパーセンタイル範囲)
  ...
  - 自動露出の測光では**空(背景)を数えません**。画作り用の調整項目は、**夜のロールオフ**(既定4.5EV)と**測光上限(基準EVから)**(既定+2.0EV)です。根拠は [docs/ImplementationDetail.md](docs/ImplementationDetail.md) 21.9.2節・21.9.6節にあります
- **ライティング** — 太陽(時刻・自動進行・方位角・太陽光のON/OFF)、月の位置、手続き空、シーン全体の露出、ライトの一覧と編集を行います
  ...
  - **積雲の「ボリュームとして描く」**(既定ON)を有効にすると、雲を雲底から雲頂までのスラブとしてレイマーチします。厚みと**レイマーチ段数**(1〜32、既定12)を指定できます。減らすと雲の内部の階調が段状に粗くなります(輪郭ではなく芯の明暗に出ます)。実測は [docs/ImplementationDetail.md](docs/ImplementationDetail.md) 35.41節にあります
    - オクターブ数と自己影の段数は実行時には変えられません。設計上の理由は [docs/Architecture.html](docs/Architecture.html) 35.12節にあります
...
- **レンダリング → ジオメトリ** — **深度プリパス**(既定ON)のON/OFF。G-Bufferを描く前に不透明ジオメトリの深度だけを先に埋め、最前面の断片だけを通します。
  - 実測と設計上の理由は [docs/ImplementationDetail.md](docs/ImplementationDetail.md) 47.17節および [docs/Architecture.html](docs/Architecture.html) 39.9節にあります。
...
- **システム** — 垂直同期、フレームレート制限(既定でON・60fps。30/60/120から選択可能)、性能のログ記録、品質プリセット、内部レンダー解像度、超解像(FSR1相当)、グラフィックスAPI、パネルの表示/非表示、レイアウトの初期化、現在のUIスケール
  ...
  - **内部レンダー解像度**(既定1920x1080) — G-Buffer以降すべての中間バッファの解像度です。**ウィンドウサイズとは独立**していて、表示時はアスペクト比を保ったままウィンドウへ拡大縮小します(余る側にレターボックス/ピラーボックスが出ます)。1280x720 / 1600x900 / 1920x1080 / 2560x1440 / 3840x2160 から選べるほか、「現在のウィンドウサイズに合わせる」で等倍表示にもできます(押した時点で1回だけ適用され、その後のウィンドウリサイズには追従しません)。上げるほどVRAM使用量とフレーム時間が増え、変更時にTAAの履歴は一度破棄されます
