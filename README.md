...
法線マッピングの接線は、モデル読み込み時にassimpの`aiProcess_CalcTangentSpace`で各頂点ごとに
計算した実接線ベクトル(UV継ぎ目で複製された頂点間は位置+法線をキーに接線を平均化して平滑化)を
頂点バッファに格納し、ピクセルシェーダーではそれを補間・再直交化してTBN行列を構築しています
(以前は画面空間微分(ddx/ddy)から近似していましたが、UV継ぎ目でピクセルクアッドがトポロジー的に
不連続になる箇所で法線が破綻するため廃止しました)。シャドウマッピングによる影の描画、
太陽光の昼夜サイクルにも対応しています。
