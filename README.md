...
- DX12バックエンドのメッシュレット描画を使う場合: メッシュシェーダー Tier 1 / シェーダーモデル6.6に対応したGPUとドライバ。加えて、実行ファイルの隣へ配布される`dxcompiler.dll`が1.6以降であること(シェーダーモデル6.6を知らない古いdxcではbindlessが無効になり、メッシュレット描画も選べなくなります)
