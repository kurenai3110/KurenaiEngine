...
`-megalightsaccum` と `-megalightsdump` は、確率的サンプリングの正しさ(平均が参照実装へ寄るか)を測るためのものです。**画面キャプチャでは測れません** — 8bitかつトーンマップ後なので、丸めだけで誤差に下限が生まれ、そこに隠れて読めなくなります。書き出したダンプは `Tools/megalights_metrics.py` で比べます(総和比・|相対誤差|の中央値・影の縁の帯・連写の時間std)。

**MegaLightsの3つの手法** — いずれもポイント/スポットライトに影を出すためのもので、太陽は対象外です。BistroExteriorNight(107灯 / 1280x720 / RTX 4070 Ti / Release)の実測:

| 手法 | 何をするか | コスト(合計 / 照明本体) | ちらつき 中央値 / p90 |
|---|---|---:|---:|
| `1` 参照実装 | 全灯を総当たりし、届く灯ごとに影レイを1本撃つ | 11.2 ms / 11.1 ms | 0.000 / 0.000 階調(決定的) |
| `2` 確率的サンプリング | 候補プールからRISで1灯選び、リザーバを時間・空間で再利用する(ReSTIR DI) | 14.5 ms / 10.4 ms | 0.44 / 1.67 階調 |
| `3` クアッド共有 | 1画素1レイのまま、2x2クアッドの4本の結果を共有して平均する | **5.2 ms** / **1.35 ms** | 0.46 / **1.21** 階調 |

「照明本体」はデノイザ(3手法共通の固定費 約3.8ms)を除いた値です。**手法3は本体だけなら参照実装の8分の1**になり、コストの74%はデノイザが占めます。ちらつきは中央値では手法2とほぼ同じですが、**裾(p90)は手法3のほうが静か**です。

コストは内部2560x1440で測った値です(1280x720ではアプリがCPU律速になり、GPUが遊んでクロックが落ちるため手法どうしで条件が揃いません — 実測でSMクロックが手法1の1771MHzに対し手法3は1167MHzでした)。**絶対値は機械の状態で1〜2%動くので、比だけを引用してください。**

**手法2は参照実装より遅くなっています。** 厳密な不偏性を保ったまま近傍のサンプルを再利用するために、再利用のたびに可視性を確かめるレイと不偏化の分母のための補正レイを撃つのが理由で、`MegaLightsSpatial` だけで 8.48 ms(合計の58%)を占めます。手法3はその再利用をやめ、代わりに2x2クアッド内で影レイの結果を共有します(UE5 MegaLights の DownsampleFactor=2 と同じ種類の近似)。影の縁が最大1画素ぼける偏りを受け入れますが、箱フィルタは積分を保存するので総和には出ません。
...
| `-megalights <番号>` | MegaLightsの手法を選ぶ(`0` = なし、`1` = 参照実装、`2` = 確率的サンプリング(ReSTIR)、`3` = クアッド共有)。DX12かつDXR Tier 1.1が要る |
| `-megalightsrays <本数>` | 参照実装が1灯あたりに撃つ影レイの本数(**既定は1**)。`0` にすると影を撃たず、従来のライトループと数値的に一致するはずの状態になる(移植の検証用)。光源に半径があるため1本では1灯1標本の確率的な推定になり、止め絵には斑点が残る。参照実装は蓄積(`-megalightsaccum`)で真値へ寄せる前提で、128枚なら32本相当に対する\|相対誤差\|のp90が0.070まで落ちる。1枚をきれいに見たいときだけ上げる(コストは本数にほぼ比例。2560x1440・107灯で1本 11.0ms → 8本 91.8ms) |
...
| `-dumptexframes <N>` | **検証専用**。**直前の** `-dumptex` をNフレーム連番で書き出す(既定 `1`)。`N > 1` のときだけ拡張子の手前に4桁が入る(`ml.bin` → `ml_0000.bin`)。1枚ずつは従来どおりの形式なので、既存のサブコマンドがそのまま使える。受け皿はリングになっており、読み戻し遅延のぶんを次のコピーが追い越さない |
| `-dumptexstride <S>` | **検証専用**。連番の捕獲間隔をエンジンのフレーム数で指定する(既定 `1`)。間隔がファイルの `FrameIndex` に残るので、後から検算できる |
...
| `-recreate <フレーム> <指示>` | **検証専用**。繰り返し指定できる。起動して回すだけではGPUリソースの作り直し経路を一度も踏まないため、指定フレームで内部レンダー解像度の変更・バッファ精度の切り替え・超解像の出力解像度の変更・シーンの切り替えを無人で踏ませる。`<指示>` は `renderres=<幅>x<高さ>`(超解像を無効にする) / `upscale=<幅>x<高さ>`(超解像を有効にして出力解像度を変える) / `precision=hdr` または `precision=legacy8bit` / `scene=<名前>`。不正な指定はログへエラーを出して読み飛ばし、起動を続ける。シーン名を解決できないときもログへエラーと候補一覧を出して続行する(起動時の `-scene` と違い、実行中なので終了コードでは返せない) |
...
| `-megalightsdenoiseframes <上限>` | 時間累積の上限フレーム数(**既定は手法2で32、手法3で64**。指定すると両方へ入る)。上げるほどちらつきは減るが、残光が 2.30*N フレームぶん尾を引く(60Hzで32なら1.2秒、64なら2.5秒)。実測が指数移動平均の理論値と3桁一致している |
| `-megalightsquadshare <0\|1>` | クアッド共有(手法3)で、2x2の仲間が撃った影レイの結果を借りるか。**既定は有効**。`0` は陽性対照で、手法2から時間・空間再利用を外した構成と**RGBがビット同一**になる |
| `-megalightsquadsamples <1〜4>` | クアッド共有(手法3)が1画素あたりに引く標本の数(**既定は4**)。**影レイの本数がそのままこれになる**。デノイザを切った1フレームの時間stdの中央値は 1本 17.95 / 2本 14.54 / 4本 12.57(階調)、MegaLights全パスの合計は 5.59 / 6.07 / 7.52 ms(2560x1440・107灯・RTX 4070 Ti・Release)。4本でも参照実装(11.3ms)より速い |
| `-megalightspool <8〜128>` | 候補プールが1タイル(16x16画素)あたりに抽出する灯の数 K(**既定は128**)。**1画素あたりの標本数では減らない「タイル間」のノイズがここで決まる** — プールはタイルに1つで、タイル内の全画素が同じK個から引くため。標本数4での1フレームの誤差は、K=32 でタイル間 7.81 / タイル内 10.27、K=128 で **4.03 / 8.30**(8bit階調・中央値)。タイル間が **-48%**。**レイの本数は増えない**ので、コストは候補プールのパスが 0.273 → 0.321 ms、全体で +1.3% に収まる |
| `-megalightsquadstratify <0\|1>` | クアッドの4画素へ候補プールのスロットを1/4ずつ割り当てるか(層化)。**既定は有効**。プールのスロットは混合分布からのi.i.d.抽出なので、割り戻しの式は変わらない |
| `-megalightsblockedcache <0\|1>` | 手法3で、遮蔽が確定した灯のキャッシュを使うか。**既定は有効**。切ると死んだ画素が 1 → 134 に増える |
| `-megalightstilejitter <0\|1\|2>` | 候補プールのタイル格子をフレームごとに画素単位でずらす(**既定は0=無効**)。`2` は検証用で「有効だがオフセットを(0,0)に固定」する対照。プールはタイルに1つなので、その抽出のばらつきがタイル内へ**共通のオフセット**として乗る。格子を振ると累積後の残差からブロック状の構造が消える。デノイザ入りで**タイル間の全オフセット最大が -14〜-16%**、一方で**画素ごとの時間stdは変わらない**(ノイズを減らすのではなく構造を散らす手法)。費用は測定の分解能以下。**評価に固定オフセットのタイル間を使わないこと** ―― 測る格子がずれるだけで下がる |
...
| `-meshlights <0\|1>` | 発光面を1点へ潰さず、**三角形のまま面積分する**(段階2)。**既定は無効**。下記の制限を読むこと |
...
どれだけ違うかは距離で決まります。実測では超過が `(√A/d)²` に比例し、**1% を切るのは
発光体の代表寸法 √A の約10倍の距離**でした(2m角パネルで約20m)。
つまり**それより近くでしか段階2は効きません。**導出と実測は
[docs/ImplementationDetail.md](docs/ImplementationDetail.md) 62.9 にあります。
