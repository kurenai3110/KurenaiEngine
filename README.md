...
1機につきカメラへ正対するビルボードを1枚、加算合成で描き、球・円環・二重らせん・格子・
ハート・らせんの6つの形を順に変形していきます。水面がある場合は編隊がそのまま映り込みます。
...
[DroneShow]
Enabled = true
Count = 1500          # 省略可(既定1500、上限4096)
Center = 0, 200, 100  # 編隊の中心(ワールド座標)
Scale = 140.0         # 編隊の代表半径[m]
Brightness = 0.30     # 実効プリ露出が掛かるため桁が小さい。0.3あたりで頭打ち
```

水面のあるシーンでは、ドローンショーを有効にすると反射の手法が自動的にスクリーンスペースへ
切り替わります。レイトレーシング反射は機体(手続き的に描くビルボード)を追跡できず、
水面に何も映らなくなるためです。手法は「レンダリング」パネルから変更できます。

すぐ試すには `Sample3D.exe -scene DroneShow` を起動してください
(`Scenes/DroneShow.kscene`。干潟の夜景の上空で1500機が飛びます)。
機体数・明るさ・変形の速さは「ドローンショー」パネルから調整できます。同パネルの
「時間を止める」と「ショー時刻」を使うと、狙った形の狙った瞬間を毎回同じように再現できます。
