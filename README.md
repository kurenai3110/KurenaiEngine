...
| `-megalightsdenoisecatmull <0\|1>` | デノイザの時間累積が履歴の色を引くときの再サンプリングを指定する(`0` = バイリニア、`1` = Catmull-Rom)。**既定は0** ―― 重みの正規化を直したあとは移動中の鮮鋭さ・誤差・総和比のすべてでバイリニアを上回るが、既定は目視のあとに決める。4タップ判定とは併用できない。根拠は docs/ImplementationDetail.md 61.7q節。 |
