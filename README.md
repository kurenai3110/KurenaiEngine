...
- **レンダリング → 水面** — 水面(`.kscene`の`[Model]Water = true`で指定したインスタンス)の法線マップのスクロールを止める「水面アニメを止める」トグルがあります。対象インスタンスは「デバッグ表示」の「水面マスク」で白く表示され確認できます。波のスケール・強さのつまみも用意していますが、現時点ではまだ描画へ反映されません。反射(SSR/RT反射との統合)は今後のフェーズで追加予定です
