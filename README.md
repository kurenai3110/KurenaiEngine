...
- メッシュ単位フラスタムカリングの間引き率は**中央値50.6%(範囲49.2〜59.7%)**で、
  これは1メッシュ=1棟というこのアセットの形によるものです(Emerald Squareでは1.5%)。
  **フレームレートはこの確認では測っていません** ―― スクリーンショットを撮りながら
  動かしたため、計測サンプルの43%が撮影の影響で落ちています
- **地形が絵に出ているかは未確認です。** 大気遠近の霞で地表が白く飛び、建物が宙に浮いて
  見えます。読み込みは成功しています(6/6モデル・Error 0)が、描画されているかは
  画像から断定できていません
...
- **プロファイラ** — FPS、CPU/GPUフレーム時間をパスごとに表示。フラスタムカリングの「判定した数 / 間引いた数」も出します(モデル単位とメッシュ単位を別々に)。**効くシーンが逆です** — 実測(DX11 / Release / 1280x720)で、Emerald Square(1モデル / 約220メッシュ)はモデル単位が0%・メッシュ単位が1.5%、PLATEAU 東京23区(671モデル / 1メッシュずつ)はモデル単位が84.0%・メッシュ単位が0%、PLATEAU 丸の内 LOD2(6モデル / 3,418メッシュ)はモデル単位が14.3%・メッシュ単位が50.6%(いずれも中央値)でした
