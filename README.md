...
SSAO/SSIL・ベイク済み遮蔽マップ(glTFの`occlusionTexture`)・SSR・複数ライト(ポイント/スポット、カンデラ/ルクス単位)・
...
- `--bake-occlusion`を付けると、あわせて**bent normal**も焼きます。遮蔽マップが「どれだけ
  ...
  [docs/Architecture.html](docs/Architecture.html) 25.10〜25.11節)。
...
> **`.kmodel`の形式が v8、`.kgeom`が v2 になりました。** v6でライトマップUVを頂点へ追加し、
> v7でbent normalのテクスチャ参照をメッシュへ追加し、v8でbent normalの格納空間を
> モデル空間から接空間へ変更したためです(v8はレイアウトが同じでテクスチャの中身の意味だけが
> 変わるため、`--bake-occlusion`を付けた再ベイクが要ります)。古い`.kmodel`はバージョン
