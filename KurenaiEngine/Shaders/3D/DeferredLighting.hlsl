...
float4 PSMain(PSInput input) : SV_TARGET
{
    ...
    {
        ambient = diffuseColor * AmbientColor.rgb * ao;
    }

    // エミッシブは自発光のためAO/シャドウの影響を受けず常に加算する。SSILの間接拡散光は
    // 拡散反射のためdiffuseColor(非金属分)を乗算してから加算する
    float3 color = ambient + diffuseColor * indirectLight + directLight + emissive;
