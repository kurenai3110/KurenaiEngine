...
//
// 【フレーム番号は意図的に混ぜていない】ピクセル座標だけの関数にしておけば、カメラが止まっている
// 限りノイズも完全に静止し、「わずかにディザが乗った静止画」で済む。
//
// TAA(TAA.hlsl)が入った現在は時間方向の平均化を行う後段が存在するため、フレームごとにノイズを
// 変えて実効サンプル数を増やす選択肢自体はある。それでもここを据え置いているのは、
// TAAの近傍クリップが「今フレームの近傍にあり得ない色」を捨てる仕組みだからで、
// 時間変動ノイズはその判定に毎フレーム引っかかって捨てられ、平均化されないまま
// ちらつきとして残りやすい。これを実用にするにはスクリーンスペースシャドウ専用の
// 空間デノイザ(分散に応じたバイラテラルブラー等)を別途用意する必要があり、
// TAAを入れただけでは成立しない。SSIL_VisibilityBitmask.hlslのstepJitterも同じ理由で
// ピクセル座標のみの関数にしている
