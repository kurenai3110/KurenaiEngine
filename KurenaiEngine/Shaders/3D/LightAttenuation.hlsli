...
//
// 【構造体にもレジスタにも依存させないこと】**このヘッダーは GPULight を受け取らず、
// スカラだけを受け取る**。ライトリストを置くレジスタは読む側ごとに違うため、
// 構造体を受け取る形にすると、どちらの経路から読まれるかで宣言が要ることになる。
// (以前は struct GPULight が二重宣言で同時インクルードできず、それもこの形の理由だった。
//  宣言は ShaderInterop/GPULight.hlsli の1本へ寄せたので、その制約のほうは解けている)
//
