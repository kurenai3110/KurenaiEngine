...
cbuffer ObjectConstants : register(b1)
{
    ...
    // このドローでHi-Zオクルージョン判定をどう行うか。
    //
    //   0 = 判定しない
    //   1 = **前フレームの**Hi-Zで判定する(PrevViewProjで投影し、球を保守的に膨らませる)
    //   2 = **今フレームの**Hi-Zで判定する(ViewProjで投影し、膨らませない)
    //
    // 【パスで分ける必要がある】Hi-Zを深度プリパスの深度から作るようにすると、
    // 同じフレームの中で「まだ作られていない(=前フレームの中身)」時点と
    // 「作り終えた(=今フレーム)」時点の両方が現れる。深度プリパスは前者、
    // G-Bufferは後者を読む。同じテクスチャなのに投影に使う行列が違うので、
    // フレーム全体のフラグでは分けられない。
    //
    // 【2は膨らませない】今フレームの深度から作ったHi-Zなら視差のずれが無く、
    // 判定はそのまま正確になる。膨らませると間引けるものを取りこぼすだけ
    uint MeshletOcclusionMode;
};
