...
float4 PSMain(PSInput input) : SV_TARGET
{
    ...
    if (clipMode > 0)
    {
        ...
        if (clipMode > 1)
        {
            ...
            // ハードに固定されたままになる(症状は「平均差分は下がるのに最大差分だけが
            // 下がらない」という形で出る)。分散側と同じrelaxで外側へ開く
    ...
    //
    // 【シャープネスをここで掛けてはいけない】アンシャープマスクを今フレームの入力へ
    // 掛けてからブレンドすると、増幅する高域が「ジッターで毎フレーム変動する成分」
    // そのものなので入力の振れ幅を直接大きくし、静止時のちらつきが大きく増える。
    // 蓄積の外(Tonemap.hlsl、トーンマップ後のLDR値)で最終出力にのみ掛ける。
