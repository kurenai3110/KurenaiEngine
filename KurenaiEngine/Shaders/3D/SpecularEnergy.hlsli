...
//
// **直接光だけ k = (roughness+1)^2 / 8 を使ってはいけない**。これはDisneyのラフネス再マップ
...
//     適用してもエネルギー補正が整合しない
// 使わないと直接光の鏡面は斜め視線・低ラフネスで明るくなる(最大5倍。roughness 1.0と
...
//
// 実機で振って測った結果、**導出値の1.0以外は使えない**(掃引の表は docs/ImplementationDetail.md 34.11.4)。
// k を上げると減衰が急すぎて、球冠交差が黒く潰していた場所を救えないまま全体を暗くする。
// 逆に下げると純黒はほぼ消えるが方向による減光まで落ち、aoB下限版で却下したのと同じ
// 「方向性が消える」失敗(34.10.2節)へ向かう
