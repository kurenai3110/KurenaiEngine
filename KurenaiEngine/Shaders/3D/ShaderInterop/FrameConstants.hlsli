...
//
// 【なぜ1本にまとめたか】以前は27本の .hlsl / .hlsli が、この宣言を
// 「先頭からの前方一致」として手で再宣言していた。宣言の深さはファイルごとに
// バラバラで(41全部のものから8つだけのものまで)、C++側の途中へフィールドを
// 挿すと後ろを宣言しているファイルが一斉にオフセットずれを起こし、
// **コンパイルは通って絵だけが静かに壊れた**。
// いまはどのシェーダーも全フィールドを宣言するので、この壊れ方は起きない
