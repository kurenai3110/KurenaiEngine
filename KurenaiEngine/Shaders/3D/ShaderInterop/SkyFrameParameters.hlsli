...
// 組み立てる。**このリポジトリで唯一の定義。**
//
// 以前は 5 本のシェーダーが同じ 20 行をそれぞれ持っており、DeferredLighting.hlsl には
// 「4つとも同時に直すこと」と書いてあった。背景・水面の映り込み・平面反射・大気遠近・
// 雲パスが同じ空を描くための同期義務を、コメントで持たせていたことになる。
// 5 本の違いは下の 2 つのマクロだけだったので、定義を 1 本にして違いをマクロへ寄せた。
//
