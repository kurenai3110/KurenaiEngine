...
// **256フレーム蓄積しても厳密に0のままの画素**が残る
// (M を増やしても減らない。実測は docs/ImplementationDetail.md 61.7f.2)。
...
// 白色は低周波成分も等量に含むので、a-trous で畳んでも「粒」として残る。
// 位相を隣どうしで離して配ると誤差が高周波側へ寄り、同じフィルタでよく落ちる
// (残差の空間自己相関の実測は docs/ImplementationDetail.md 61.7g)。
//
