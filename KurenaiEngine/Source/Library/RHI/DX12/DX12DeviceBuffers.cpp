...
namespace Kurenai::RHI
{
    ...
    std::unique_ptr<IRHIBuffer> DX12Device::CreateStructuredBuffer(const BufferDesc& desc)
    {
    ...
    std::unique_ptr<IRHIBuffer> DX12Device::CreateStructuredRWBuffer(const BufferDesc& desc)
    {
    ...
    std::unique_ptr<IRHIBuffer> DX12Device::CreateStructuredImmutableBuffer(const BufferDesc& desc)
    {
        if (desc.StrideInBytes == 0)
        {
    ...
    std::unique_ptr<IRHIBuffer> DX12Device::CreateStructuredReadOnlyBuffer(const BufferDesc& desc)
    {
        const CD3DX12_HEAP_PROPERTIES heapProps(D3D12_HEAP_TYPE_DEFAULT);
    ...
    std::unique_ptr<IRHIBuffer> DX12Device::CreateReadbackBuffer(const BufferDesc& desc)
    {
    ...
    std::unique_ptr<IRHIBuffer> DX12Device::CreateIndirectArgsBuffer(const BufferDesc& desc)
    {
    ...
    // マップしたままにする(従来通り)
    std::unique_ptr<IRHIBuffer> DX12Device::CreateConstantRingBuffer(const BufferDesc& desc)
    {
    ...
    std::unique_ptr<IRHIBuffer> DX12Device::CreateVertexIndexBuffer(const BufferDesc& desc)
    {
        const CD3DX12_HEAP_PROPERTIES defaultHeapProps(D3D12_HEAP_TYPE_DEFAULT);
        const CD3DX12_RESOURCE_DESC defaultResourceDesc = CD3DX12_RESOURCE_DESC::Buffer(desc.SizeInBytes);
        Microsoft::WRL::ComPtr<ID3D12Resource> resource;
    ...
    std::unique_ptr<IRHIBuffer> DX12Device::CreateBuffer(const BufferDesc& desc)
    {
        ...
        // 【ロックはここで取る】下の各Create*は、このロックを保持したまま呼ばれることを前提に
        // 書いてある(std::mutexは再帰ロックできないため、切り出し先で取り直してはいけない)
        std::lock_guard<std::mutex> uploadLock(m_UploadMutex);

        // Usageごとにヒープ種別・初期状態・ステージングリングの段数が違う。
        // 取り違えても多くのUsageは動いてしまい、特定の経路でだけ壊れるため、
        // 分岐はここ1か所に集め、各Usageの手順は独立した関数に閉じてある
        switch (desc.Usage)
        {
        case BufferUsage::Structured:           return CreateStructuredBuffer(desc);
        case BufferUsage::StructuredRW:         return CreateStructuredRWBuffer(desc);
        case BufferUsage::StructuredImmutable:  return CreateStructuredImmutableBuffer(desc);
        case BufferUsage::StructuredReadOnly:   return CreateStructuredReadOnlyBuffer(desc);
        case BufferUsage::Readback:             return CreateReadbackBuffer(desc);
        case BufferUsage::IndirectArgs:         return CreateIndirectArgsBuffer(desc);
        case BufferUsage::Constant:             return CreateConstantRingBuffer(desc);
        default:
            // Vertex/Indexと、上のいずれにも当たらないUsage。分割前も同じく末尾へ落ちていた
            return CreateVertexIndexBuffer(desc);
        }
    }
}
