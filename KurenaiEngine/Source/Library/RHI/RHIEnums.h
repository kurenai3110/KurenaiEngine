...
namespace Kurenai::RHI
{
    ...
    enum class Format
    {
        ...
        R11G11B10_Float,
        // リニア値をsRGBカーブで格納する8bitフォーマット。GPUが書き込み時にエンコード・
        // 読み取り時にデコードするため、シェーダ側は常にリニア値として扱えばよい。
        // 同じ8bitでも符号点を暗部へ寄せられるので、暗い値ほど量子化が細かくなる
        // (リニア反射率L=0.02で約4.3倍、L=0.01で約6.5倍細かい)。
        // ただしL>0.244では逆に粗くなるため、暗い値を格納するバッファにのみ使うこと
        R8G8B8A8_UNorm_SRGB,
    };
