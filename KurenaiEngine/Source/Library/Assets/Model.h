...
namespace Kurenai::Assets
{
    ...
    struct EmissiveCluster
    {
        ...
        // 面積等価の円板半径 sqrt( 2 * Σ(A_i |c_i - C|^2) / ΣA_i )。
        // 一様な円板の2次モーメントが a^2/2 であることから決めている。
    ...
    struct Mesh
    {
        ...
        //
        // 【sRGB→線形は平均の前に行う】逆順(平均してから EOTF)にすると、暗い背景に明るい
        // グリフが乗った看板で真値0.5に対して0.214が出る(2倍以上暗い)。
        //
        // 【UVの範囲で切り出す】アトラスの一角しか使わないメッシュでは、テクスチャ全体の平均が
        // 無関係な部分の色を拾う。ModelLoader が頂点走査で求めたUVのAABBで矩形平均を取る
