        ...
        Rendering::RenderBlackboard& bb)
    {
        ...
        if (lightCullingRuns)
        {
            graph.AddPass(Core::RenderGraphPassDesc{
                ...
                .Execute = [this, lightBuffer, &gpuLights, viewMatrix, jitteredProj, renderWidth, renderHeight](RHI::IRHICommandList* cmd)
                {
                    ...
                    cmd->UpdateBuffer(m_Engine.m_LightCullingConstantBuffer.get(), &cullingConstants, sizeof(cullingConstants));
        ...
        if (megaLightsRuns && m_Engine.m_MegaLightsTilePoolBuffer && m_Engine.m_MegaLightsTilePoolPipelineState)
        {
            graph.AddPass(Core::RenderGraphPassDesc{
                ...
                .Execute = [this, lightBuffer, megaLightsSettings, &gpuLights, viewMatrix, jitteredProj, megaLightsEffectiveTilesX, megaLightsEffectiveTilesY, megaLightsTileOffset, renderWidth, renderHeight](RHI::IRHICommandList* cmd)
                {
                    ...
                    cmd->UpdateBuffer(m_Engine.m_MegaLightsTilePoolConstantBuffer.get(), &poolConstants, sizeof(poolConstants));
        ...
        const bool meshLightsActive = m_Engine.m_MeshLightsEnabled && m_Engine.m_MeshLightScene.IsValid();
        const uint32_t meshLightTriangleCount =
            meshLightsActive ? m_Engine.m_MeshLightScene.GetTriangleCount() : 0u;
        ...
        RHI::IRHIBuffer* const meshLightBufferForBinding =
            meshLightsActive ? m_Engine.m_MeshLightScene.GetTriangleBuffer() : lightBuffer;
        ...
        if (megaLightsRuns && frame.Settings.MegaLights.Mode == MegaLightsMode::Reference)
        {
            graph.AddPass(Core::RenderGraphPassDesc{
                ...
                .Execute = [this, lightBuffer, raytracingScene, brdfLUTTexture, emissiveLightSettings, megaLightsSettings, &gpuLights, tilePoolBufferForBinding, meshLightBufferForBinding, meshLightTriangleCount, renderWidth, renderHeight, frameConstantBuffer, screenSpaceSamplers](RHI::IRHICommandList* cmd)
                {
                    ...
                    megaLightsConstants.Params1 = { m_Engine.m_TAAFrameIndex, meshLightTriangleCount, 0u, 0u };
            ...
            (frame.Settings.MegaLights.Mode == MegaLightsMode::Stochastic || megaLightsQuadShared))
        {
            ...
            const auto updateStochasticConstants = [this, buildStochasticConstants](RHI::IRHICommandList* cmd)
            {
                ...
                cmd->UpdateBuffer(
                    m_Engine.m_MegaLightsStochasticConstantBuffer.get(), &stochasticConstants, sizeof(stochasticConstants));
            };
            ...
            const uint32_t historyWriteIndex = m_Engine.m_MegaLightsHistoryIndex;
            const uint32_t historyReadIndex = m_Engine.m_MegaLightsHistoryIndex ^ 1u;
            ...
            RHI::IRHIBuffer* const temporalOutputBuffer =
                temporalRuns ? m_Engine.m_MegaLightsReservoirHistory[historyWriteIndex].get() : nullptr;
            ...
            graph.AddPass(Core::RenderGraphPassDesc{
                ...
                .Execute = [this, lightBuffer, raytracingScene, brdfLUTTexture, tilePoolBufferForBinding, updateStochasticConstants, renderWidth, renderHeight, frameConstantBuffer, screenSpaceSamplers](RHI::IRHICommandList* cmd)
                {
            ...
            if (temporalRuns)
            {
                graph.AddPass(Core::RenderGraphPassDesc{
                    ...
                    .BufferReads = { lightBuffer, m_Engine.m_MegaLightsReservoirBuffer.get(),
                                     m_Engine.m_MegaLightsReservoirHistory[historyReadIndex].get(),
                    ...
                    .Execute = [this, lightBuffer, raytracingScene, brdfLUTTexture, historyReadIndex, historyWriteIndex, renderWidth, renderHeight, frameConstantBuffer, screenSpaceSamplers](RHI::IRHICommandList* cmd)
                    {
            ...
            for (uint32_t spatialIteration = 0u; spatialIteration < spatialIterations; ++spatialIteration)
            {
                ...
                graph.AddPass(Core::RenderGraphPassDesc{
                    ...
                    .Execute = [this, lightBuffer, raytracingScene, brdfLUTTexture, spatialInput, spatialOutput, spatialConstants, spatialIteration, buildStochasticConstants, renderWidth, renderHeight, frameConstantBuffer, screenSpaceSamplers](RHI::IRHICommandList* cmd)
                    {
            ...
            if (megaLightsQuadShared)
            {
                ...
                RHI::IRHIBuffer* const guideWriteBuffer = m_Engine.m_MegaLightsHistoryGuide[historyWriteIndex].get();
                graph.AddPass(Core::RenderGraphPassDesc{
                    ...
                    .Execute = [this, lightBuffer, brdfLUTTexture, guideWriteBuffer, renderWidth, renderHeight, frameConstantBuffer, screenSpaceSamplers](RHI::IRHICommandList* cmd)
                    {
            ...
            graph.AddPass(Core::RenderGraphPassDesc{
                ...
                .Execute = [this, lightBuffer, raytracingScene, brdfLUTTexture, shadeReservoirBuffer, renderWidth, renderHeight, frameConstantBuffer, screenSpaceSamplers](RHI::IRHICommandList* cmd)
                {
                    // 定数はInitial側で更新済み。ここでバインドし直すのは、DX12が
                    // SetPipelineStateのたびにルート引数を無効化するため
        ...
        {
            const uint32_t denoiseWrite = m_Engine.m_MegaLightsDenoiseHistoryIndex;
            ...
            RHI::IRHIBuffer* const denoiseGuideBuffer =
                m_Engine.m_MegaLightsHistoryGuide[m_Engine.m_MegaLightsHistoryIndex ^ 1u]
                    ? m_Engine.m_MegaLightsHistoryGuide[m_Engine.m_MegaLightsHistoryIndex ^ 1u].get()
                    : nullptr;
            ...
            graph.AddPass(Core::RenderGraphPassDesc{
                ...
                .Writes =
                {
                    m_Engine.m_MegaLightsDenoisePing[0].get(), m_Engine.m_MegaLightsDenoiseMomentPing[0].get(),
                    m_Engine.m_MegaLightsDenoiseHistory[denoiseWrite].get(),
                    m_Engine.m_MegaLightsDenoiseMoments[denoiseWrite].get(),
                },
                ...
                .Execute = [this, denoiseRead, denoiseWrite, updateDenoiseConstants, bindDenoiseCommon, renderWidth, renderHeight](RHI::IRHICommandList* cmd)
                {
                    ...
                    cmd->SetComputeTexture(6, m_Engine.m_MegaLightsTexture.get());
                    // 【読むのは前フレームが書いた側】今フレームはもう片方へ書くので
                    // 同一フレーム内でのWARが生じない(RenderGraphはWARの辺を張らない)
                    cmd->SetComputeTexture(7, m_Engine.m_MegaLightsDenoiseHistory[denoiseRead].get());
                    cmd->SetComputeTexture(8, m_Engine.m_MegaLightsDenoiseMoments[denoiseRead].get());
                    cmd->SetComputeUnorderedAccessTexture(0, m_Engine.m_MegaLightsDenoisePing[0].get());
            ...
            for (int atrousPass = 0; atrousPass < atrousPasses; ++atrousPass)
            {
                ...
                graph.AddPass(Core::RenderGraphPassDesc{
                    ...
                    .Execute = [this, atrousSrc, atrousDst, atrousPass, atrousStep, updateDenoiseConstants, bindDenoiseCommon, renderWidth, renderHeight](RHI::IRHICommandList* cmd)
                    {
                        ...
                        bindDenoiseCommon(cmd);
                        cmd->SetComputeTexture(6, m_Engine.m_MegaLightsDenoisePing[atrousSrc].get());
                        // t7は使わないが、DX12は宣言したリソースを全部束縛しないと壊れる
                        cmd->SetComputeTexture(7, m_Engine.m_MegaLightsDenoisePing[atrousSrc].get());
                        cmd->SetComputeTexture(8, m_Engine.m_MegaLightsDenoiseMomentPing[atrousSrc].get());
                        cmd->SetComputeUnorderedAccessTexture(0, m_Engine.m_MegaLightsDenoisePing[atrousDst].get());
                        cmd->SetComputeUnorderedAccessTexture(
                            1, m_Engine.m_MegaLightsDenoiseMomentPing[atrousDst].get());
            ...
            graph.AddPass(Core::RenderGraphPassDesc{
                ...
                .Execute = [this, denoiseFinalSrc, updateDenoiseConstants, bindDenoiseCommon, renderWidth, renderHeight](RHI::IRHICommandList* cmd)
                {
                    ...
                    bindDenoiseCommon(cmd);
                    cmd->SetComputeTexture(6, m_Engine.m_MegaLightsDenoisePing[denoiseFinalSrc].get());
                    cmd->SetComputeTexture(7, m_Engine.m_MegaLightsDenoisePing[denoiseFinalSrc].get());
                    cmd->SetComputeTexture(8, m_Engine.m_MegaLightsDenoiseMomentPing[denoiseFinalSrc].get());
                    cmd->SetComputeUnorderedAccessTexture(0, m_Engine.m_MegaLightsDenoisedTexture.get());
        ...
        RHI::IRHITexture* const megaLightsAccumSourceTexture =
            (megaLightsDenoiseRuns && m_Engine.m_MegaLightsDenoisedTexture) ? m_Engine.m_MegaLightsDenoisedTexture.get()
                                                                   : m_Engine.m_MegaLightsTexture.get();
        ...
        if (megaLightsAccumRuns)
        {
            ...
            const uint32_t accumReset = (m_Engine.m_MegaLightsAccumFrames == 0u) ? 1u : 0u;
            graph.AddPass(Core::RenderGraphPassDesc{
                ...
                .Execute = [this, accumReset, megaLightsAccumSourceTexture, renderWidth, renderHeight](RHI::IRHICommandList* cmd)
                {
                    ...
                    cmd->SetComputeConstantBuffer(0, m_Engine.m_MegaLightsAccumConstantBuffer.get());
            ...
            m_Engine.m_MegaLightsAccumFrames >= static_cast<uint32_t>(frame.Settings.MegaLights.AccumTargetFrames))
        {
            ...
            {
                if (!m_Engine.m_MegaLightsAccumReadback)
                {
                    try
                    {
                    ...
                    catch (const std::exception& e)
                    {
                        ...
                        m_Engine.m_MegaLightsDumpDone = true; // 何度も試さない
                    }
                }
                ...
                if (m_Engine.m_MegaLightsAccumReadback)
                {
                    graph.AddPass(Core::RenderGraphPassDesc{
                        .Name = "MegaLightsDump",
                        .BufferReads = { m_Engine.m_MegaLightsAccumBuffer.get() },
                        .Execute = [this, accumBytes](RHI::IRHICommandList* cmd)
                        {
                            cmd->CopyBufferToReadback(
                                m_Engine.m_MegaLightsAccumReadback.get(), m_Engine.m_MegaLightsAccumBuffer.get(), accumBytes);
                        },
                    });
                    m_Engine.m_MegaLightsDumpIssued = true;
                    m_Engine.m_MegaLightsDumpCopyFrame = m_Engine.m_TAAFrameIndex;
                }
            }
            ...
            else if (m_Engine.m_TAAFrameIndex - m_Engine.m_MegaLightsDumpCopyFrame >= 5u)
            {
                    ...
                    else
                    {
                        Core::Logger::Error(
                            ...
                            "MegaLightsの蓄積平均を書き出せませんでした(ファイルを開けない): " +
                                Core::WideToUtf8(m_Engine.m_MegaLightsDumpPath));
                    }
                }
                else
                {
                ...
                m_Engine.m_MegaLightsDumpDone = true;
            }
        }
    }
}
