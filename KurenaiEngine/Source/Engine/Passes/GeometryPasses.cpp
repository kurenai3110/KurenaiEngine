...
namespace Kurenai::Passes
{
        ...
        const std::vector<RHI::InputElementDesc>& modelInputLayout)
    {
        ...
        // メッシュシェーダー版の深度プリパス。
        //
        // 【これが無いとプリパスがまるごと止まる】プリパスとG-Bufferで別の経路を使うと、
        // 同じ頂点でも変換の丸めが一致する保証が無く、深度がずれた面が深度テストを通らずに
        // 消える。**同じ増幅/メッシュシェーダーをそのまま使えばこの問題自体が消える**
        // (経緯は docs/ImplementationHistory.md 53.1)。
        //
        ...
        Rendering::RenderBlackboard& bb)
    {
        ...
        // 排他だった。プリパスにもG-Bufferとまったく同じ増幅/メッシュシェーダーを使うPSOを
        // 用意したので、変換は文字どおり同一のコードになりこの問題は起きない
        // (経緯は docs/ImplementationHistory.md 53.1)。
        //
