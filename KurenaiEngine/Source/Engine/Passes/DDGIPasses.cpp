...
namespace Kurenai::Passes
{
    ...
    void DDGIPasses::CreateResolvePipelineState(RHI::IRHIDevice& device, const std::wstring& shaderDirectory)
    {
        ...
        m_DDGIResolveVertexShader = device.CreateShader(ddgiResolveVsDesc);
        ...
        m_DDGIResolvePixelShader = device.CreateShader(ddgiResolvePsDesc);
        ...
        m_DDGIResolvePipelineState = device.CreatePipelineState(ddgiResolvePipelineDesc);
    }

    void DDGIPasses::CreateRaytracedTraceResources(RHI::IRHIDevice& device, const std::wstring& shaderDirectory)
    {
        ...
        m_DDGIProbeTraceComputeShader = device.CreateShader(ddgiTraceCsDesc);
        m_DDGIProbeTracePipelineState =
            device.CreateComputePipelineState({ m_DDGIProbeTraceComputeShader.get() });
        ...
        m_DDGITraceConstantBuffer = device.CreateBuffer(ddgiTraceConstantBufferDesc);
    }

    void DDGIPasses::ResetRaytracedTraceResources()
    {
        // HasRaytracedTraceResources がパイプラインステートのnullを見ているため、
    ...
    void DDGIPasses::CreateCaptureResources(RHI::IRHIDevice& device)
    {
    ...
    void DDGIPasses::CreateProbeUpdateResources(RHI::IRHIDevice& device, const std::wstring& shaderDirectory)
    {
        ...
        m_DDGIProbeUpdateComputeShader = device.CreateShader(ddgiUpdateCsDesc);
        m_DDGIProbeUpdatePipelineState = device.CreateComputePipelineState({ m_DDGIProbeUpdateComputeShader.get() });
        ...
        m_DDGIBorderCopyComputeShader = device.CreateShader(ddgiBorderCsDesc);
        m_DDGIBorderCopyPipelineState = device.CreateComputePipelineState({ m_DDGIBorderCopyComputeShader.get() });
        ...
        m_DDGIUpdateConstantBuffer = device.CreateBuffer(ddgiUpdateConstantBufferDesc);
        ...
        m_DDGIInvalidateProbesComputeShader = device.CreateShader(ddgiInvalidateCsDesc);
        m_DDGIInvalidateProbesPipelineState =
            device.CreateComputePipelineState({ m_DDGIInvalidateProbesComputeShader.get() });
    ...
    void DDGIPasses::ResetProgress(uint32_t probeCount)
    {
        // 確保し直した直後のアトラスは中身が未定義なので、全スロットを「未確定」として持つ。
        // 【あり得ない座標で埋める】0で埋めると、たまたまその座標を担当するスロットが
        // 「もう焼いてある」と誤判定される
        m_DDGIProbeBakedCoord.assign(probeCount, DirectX::XMINT3{ INT32_MIN, INT32_MIN, INT32_MIN });
        ...
        const Rendering::RenderBlackboard& bb)
    {
            ...
            Rendering::ForEachGeometryDraw(
                    ...
                    const Assets::Mesh& mesh, float)
                {
                    ...
                    const bool instanceHasProxy =
                        unit.InstanceIndex < m_Engine.GetEmissiveProxyInstances().size()
                        && m_Engine.GetEmissiveProxyInstances()[unit.InstanceIndex];
        ...
        if (frame.Settings.DDGI.Enabled && gi->HasGIVolume && gi->DDGIProbeCount > 0 && !m_DDGIUpdateSuspended)
        {
            ...
            uint32_t perFrame = std::min<uint32_t>(
                static_cast<uint32_t>(std::max(frame.Settings.DDGI.ProbesPerFrame, 1)), gi->DDGIProbeCount);
            ...
            // (理由はDDGIPasses.hのm_DDGIOverwriteRemainingのコメント参照)。
            ...
            // この数を見れば取り違えにすぐ気づける
            if (gi->GIVolume.FollowCamera && !m_DDGIDirtyProbeList.empty())
            {
                ...
                Core::Logger::Info(
                    ...
                    "DDGIの格子がスクロールしました: 未確定 " +
                        ...
                        std::to_string(gi->DDGIProbeCount) + " スロット(このフレームで焼き直すのは " +
            ...
            for (uint32_t i = 0; i < perFrame; ++i)
            {
                ...
                const uint32_t probeIndex = isDirtySlot
                    ...
                    : (m_DDGIUpdateCursor + (i - dirtyThisFrame)) % gi->DDGIProbeCount;
            ...
            const bool cycleCompleted = nextCursor >= gi->DDGIProbeCount;
            ...
            if (warmingUp && cycleCompleted)
            {
                ...
                Core::Logger::Info(
                    "KurenaiEngine3D",
                    "DDGIの初回一巡が完了しました(" + std::to_string(gi->DDGIProbeCount) + "プローブ)");
            }
            m_DDGIUpdateCursor = nextCursor % gi->DDGIProbeCount;
        }
    }
        ...
        Core::RenderGraph& graph, const Rendering::RenderFrameContext& frame)
    {
        ...
        const bool ddgiResolvePassRuns =
            frame.Settings.DDGI.HalfResolution && gi->DDGIResolveTexture && frame.Settings.DDGI.Enabled && gi->HasGIVolume && m_DDGIBaked;
        if (ddgiResolvePassRuns)
        {
            RHI::Viewport ddgiResolveViewport;
            ddgiResolveViewport.Width = static_cast<float>(gi->DDGIResolveWidth);
            ddgiResolveViewport.Height = static_cast<float>(gi->DDGIResolveHeight);
