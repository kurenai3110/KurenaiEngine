...
namespace Kurenai
{
    ...
    void KurenaiEngine3D::UpdateEffectiveExposure(float keyIlluminanceLux)
    {
        ...
        // 露出はTonemap/Bloom/AutoExposureが同じ値で割り戻すため、これを動かしても絵は変わらない
        {
            ...
            // 【下限は表示レンジの両端をfp16へ収める条件から決まる】このバイアスは
            // 「HDRバッファの値」と「トーンマップが受け取る表示値」の橋渡しなので、
            // fp16の上限と最小正規化数がそのまま**表示できる明るさの上限と下限**になる。
            // 導出は docs/ImplementationDetail.md 38.9。**要件が表示値で書けているので、
            // シーンのExposureにもその夜の照度にも依存しない**のがこの値の性質である。
