...
// パス群が読む値をまとめたスナップショット(段階6)。大半はフレームの先頭で確定するが、
// 6フィールド(GBufferViewport / ShadowViewport / BakedLightCount / ProbeFaceProjection /
// ProbeCaptureReads / CascadeViewProj)だけは KurenaiEngine3D::RegisterPasses が
// **最初の登録の後、2番目の登録の前**に埋める。**これらを読む群より前で書くこと。**
//
// 【なぜスナップショットにするか】パス群を別クラスへ出すと、これらの値は
// 「引数で渡すもの」になる。個別の引数で渡すと、パス群が増えるたびに
...
//
// 【Blackboard との違い】こちらは**群にとっての入力**で、群は書き換えない
// (上の6フィールドを埋めるのは群ではなく RegisterPasses)。
// **群そのものが登録の途中で書く出力**は Rendering::RenderBlackboard が持つ。
namespace Kurenai
{
    struct SunLighting;
    struct GPULight;
