...
namespace Kurenai::Defaults
{
    ...
    // 既定で無効なのは、現段階の実装が参照実装(全灯総当たり)しか無く、
    // ライト数に比例して重いため。確率的サンプリング本体が入るまでは検証用の経路として扱う
    ...
    // punctualは光源方向が1つに決まるため、1より大きくしても答えは変わらない
    ...
    // 【既定は無効】一時は「入れると球が倍以上悪化する」実測が出たが、原因は結合ではなく
    // 初期サンプル側のバグ(空リザーバに M=0 を書いていて分母が過小になる)で、修正済み。
    // 修正後の実測では不偏(総和の相対差が空間なしと同水準)で、誤差中央値と裾(p99)は
    // 改善するが、影の縁(p95)と球の中央値はまだ空間なしをわずかに下回る。
    // 明確な得になっていない段階でコスト(約1ms @1280x720/RTX 4070 Ti)を既定で払わない。
    // 時間再利用とデノイザが入った時点で再評価する。数値と経緯は
    // docs/ImplementationDetail.md 61.7 と docs/ImplementationHistory.md 67章
    inline constexpr bool MegaLightsSpatialEnabled = false;
    ...
    // RTXDI系では標準の段だが、実測で不利と分かったため既定は無効。
    //   ・空間再利用と両立しない: 殺された画素の実効的な定義域が p̂ から p̂・可視率 へ
    //     変わるが、不偏化の分母(Z)はレイを撃たずに可視率を判定できず、影の縁が暗く偏る
    //     (実測: 総和の相対差 -0.54% / 殺し無しは -0.29% で不偏、p95も殺し無しが良い)
    //   ・空間再利用なしでは純粋な無駄: 殺されるサンプルはシェード側のレイでもどうせ0で、
    //     可視なサンプルではレイが2本になるだけ(絵は1bitも変わらない。実測で確認済み)
