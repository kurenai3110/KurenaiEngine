...
namespace Kurenai::Defaults
{
    ...
    //
    // 【何が壊れていたか】判定はカメラ自身の移動による深度変化を差し引いていなかった。
    // 1フレームの前進量 v に対し、**視距離 z < v / 0.05 の画素は遮蔽が変わらなくても
    // 履歴を捨てられる**。キー操作の移動速度 5.00 m/s なら v = 0.0833m で、1.67m 以内すべて。
    // 実測(5 m/s で前進、K=32、バイリニア参照 ON):
    //   棄却率           20.3〜58.5%  ->  3.3〜4.7%(残るのは本物の遮蔽変化)
    //   履歴長の中央値   46.6〜1.0    ->  64.0(上限)
    //   隣接タイル段差   0.0869       ->  0.0376
    //   N1 ノイズ        6.2589       ->  3.5522
    //   N2 ちらつき      31.562       ->  17.24
    //   S 鮮鋭度         0.82357      ->  0.82318(**動かない。ぼかして稼いでいない**)
    //
    // 【残件】長い履歴の帯で符号つき誤差が +0.0143 -> +0.0256 へ増える。これは遅れで、
    // 5 m/s で64フレーム累積すれば 5.5m 手前の情報が混ざるため。総和比の差は +0.37% で
    // 許容(±0.5%)の内側。「誤差が正の画素の割合」が 53.7% -> 58.9% へ動くのは
    // **母集団の入れ替わり**で、OFF では画素の 39.5% が棄却直後(誤差が負)だったのが
    // 4.0% まで減るため ―― 帯ごとに見れば新しい偏りは入っていない。
    //
    ...
    // --- 時間累積の履歴長を適応させる ---
    // 【何を解こうとしているか】時間累積の上限フレーム数は、上げるほどノイズが下がり
    // (手法3で 64→128 のとき連写の時間std中央値 0.451→0.299。ほぼ理論の1/√2)、
    // 上げるほどゴーストが伸びる(消灯後32フレームの残光 77.8%→88.0%)。実測は指数移動平均の
    // 理論値と3桁一致しており、**定数1つでは既定が決まらない**(docs/ImplementationDetail.md 61.7j.6)。
    // ここは、そのトレードオフを「定常の窓の長さ」と「変化への追従」の2本に割るための入口である。
    // 履歴長を定数ではなく「変化が起きていないあいだだけ伸びる状態量」にする。
    //
    // 【既定が0なのは、まだ測っていないから】0にすると乗じる係数が 1.0f になり、
    // 従来の式へ厳密に還元される(IEEEで 1.0f 倍と 1.0f-0.0f は厳密)。
    // **これが陽性対照そのもの** ―― 0で従来とビット同一にならないなら、
    // 以降どんな数値を出しても意味がない。掃引して根拠が出たら既定を動かすこと。

    // 幾何の部分減衰の強さ。再投影先の4タップは、しきい値の内側なら「通った」として
    // 同じ重みで長い履歴を主張する。しきい値で1になるよう正規化した不一致度に応じて
    // 履歴長を連続的に縮める。1でしきい値まで線形。被覆(通ったタップのバイリニア重みの和)も掛かる
    inline constexpr float MegaLightsDenoiseGeometryFalloff = 0.0f;
    // 時間勾配で履歴を縮める強さ。0で無効(従来と一致)、1でz1到達時に履歴を完全に捨てる
    inline constexpr float MegaLightsDenoiseGradientStrength = 0.0f;
    // 変化とみなし始める相対変化。タイル内平均の差を明るさで割った値。
    // **静止シーンの偽陽性率で決める値。**
    // 【標準誤差で正規化する形は測って落とした】σ/√n で割る検定にすると、完全に静止した
    // シーンで履歴長の中央値が 128 → 75.9 へ落ち、画素の時間stdが 1.215 → 2.472 と倍になった。
    // タイル内の画素の誤差は独立ではない(候補プールがタイルに1つ)ため、
    // 標準誤差を 1/√n で見積もると過小になり、z が常に過大に出る。
    // 経緯と実測は MegaLightsDenoise.hlsl の該当コメント
    inline constexpr float MegaLightsDenoiseGradientRelStart = 0.35f;
    // ここまで来たら履歴を捨てきる。**追従の速さ(-megalightsperturb の残光)で決める値**
    inline constexpr float MegaLightsDenoiseGradientRelFull = 0.6f;
    // 相対変化を比べる前に現フレームの平均を均す「速いEMA」の長さ(フレーム数)。0で無効。
    // 1フレームの平均はまだノイジーで、そのまま比べると相対変化が揺れる
    inline constexpr int MegaLightsDenoiseGradientFastFrames = 4;
    // 一度撃ったタイルが撃ち続ける長さ(フレーム数)。0で無効。
    // 【無いと「まだら」になる】部分的にしか撃てなかったタイルは、履歴が中途半端に暗く
    // なった時点で相対変化がしきい値を下回り、それ以上撃たなくなる ―― 自分で自分を止める罠で、
    // 取り残された領域が前の明るさのまま残る。実測では、全消灯の2フレーム後の λ が
    // 中央値 0.96 に対し p1 が 0.67・最小 0.077 で、この下側の裾がそのまま画面のまだらになった。
    // 【長すぎると追従が鈍る方向ではなく、静止時の偽陽性が伸びる】1回の誤発火が
    // この長さだけ尾を引くので、静止シーンの偽陽性率と対で決める値である
    inline constexpr int MegaLightsDenoiseGradientLatchFrames = 8;
    ...
    // クアッド共有(手法3)が標本を借りる範囲の半径。1 = 2x2、2 = 4x4。
    //
    // 【何と引き換えなのか】項の数は (2*半径)^2 × 1画素あたりの標本数で決まる。
    // 半径2・標本1は、半径1・標本4と**同じ16項**でありながら、
    // Initial の仕事(RIS の候補評価と影レイ)が 1/4 になる。
    // レイの発射点はむしろ 4 点から 16 点へ増えるので、球光源の可視性の分散には有利。
    // 代償は借りる距離で、V_j を V(x, y_j) の代用に使う近似の誤差が
    // 対角 sqrt(2) 画素から sqrt(18) 画素へ広がる(硬い影の縁がぼける)。
    //
    // **既定は1(従来のまま)。** 半径2が画質として許容できるかは実測で決める
    inline constexpr int MegaLightsQuadShareRadius = 1;
    ...
    // ばらつき・なまり・総和比は悪化しない(ぼかして稼いだのではない)。
    ...
    //
    // 【何が起きていたか】IGN は「隣接画素の値が離れる」ことしか保証せず、等値線が直線なので
    // **位相の場が等方ではない**。角度エネルギーの偏り(周期4〜32画素・5度ビンの最大比。
    // 等方は 0.028)は IGN 0.36 / 白色 0.031 / ブルーノイズ 0.051。
    // デノイザを切ると、描いた絵にも 27.5度 の斜めの筋として出る。
    //
    // 【ただし初期候補数 M が小さいときだけ】候補スロットは位相で決まるが、そこから
    // どれを採るかは白色の採用判定が決めるので、M が大きいと洗い流される。
    // 実測(SceneColor・デノイザ切・平らな壁・順位変換後の角度最大ビン):
    //   M=1  IGN 0.069〜0.075(27.5度) / 白色 0.031〜0.034 / ブルーノイズ 0.030〜0.034
    //   M=8  相関比 eta^2 = 0.00002 で陰性対照 0.00004 と差が無い(何も出ない)
    //
    // 【デノイザを通すと向きは消える。残るのはちらつきの差】a-trous 後はどれもピーク角が
    // 0〜5度へ移る。精度の差は同一構成2回のばらつき(±0.85%)の内側。
    // ちらつき(時間std)は下限 ±0.22% に対し、IGN 比で 白色 +5.4% / ブルーノイズ -7.4%。
    //
    // 【既定は 0(IGN)のまま。目視の判断】時間累積の上限が既定(64)のままでは、
    // ブルーノイズとの差は**目で見て分からない**。上限を下げると見えるようになる。
    // 常用の設定で見えないものを既定にはしない。低い上限で使うときの選択肢として残す。
    //
    // 1(白色)は既定の候補ではない ―― IGN より悪い。「斜めの筋が位相から来ている」ことを
    // 示した対照で、主張をやり直せるように残してある。
    // 数値と測定条件は docs/ImplementationDetail.md 61.7x
    inline constexpr int MegaLightsNoiseMode = 0;
    // --- 可視灯リスト(提案分布の第3成分) ---
    //
    // 候補プールの重みは距離減衰だけで決まり、**可視性を一切見ていない**。影の縁では
    // 目標関数を支配する灯が自分からは遮蔽されていることがあり、RISは毎フレームその灯を
    // 選んでは殺される(デノイズ前の暗黒点の主因)。前フレームにタイルの中で実際に
    // 可視だった灯を覚えておき、次フレームの提案分布へ混ぜて標本を届く灯へ寄せる。
    //
    // 【既定は無効 ―― 効くが無料ではない】遅れ(残像)は実測で出なかったが、
    // **GPU時間が +6.6% 増える**。効果と引き換えに払う量なので、既定では入れない。
    //
    // 実測(RTX 4070 Ti / DX12 / BistroExteriorNight / 1920x1080 / Release / 手法3 /
    //       -megalightspool 32 / デノイザ切 / c=0.5。詳細は docs/ImplementationDetail.md):
    //   暗黒点(点灯画素あたり)  7.78% → 4.10%(c=0.75 で 2.91%)
    //   |相対誤差| 中央値        0.01543 → 0.01468(900枚蓄積・参照実装比)
    //   GPU合計                  6.148ms → 6.554ms (+0.406ms / +6.6%。同一構成2回の
    //                            ばらつきは 0.036ms なので、この差はノイズではない)
    //   遅れ                     横滑り・前進2m/s・前進6m/s のいずれでも ON のほうが
    //                            真値に近い(遅れは増えていない)
    ...
    //
    // 【実測で決めてある】構築パスは打ち切る前の「相異なる可視灯の数」をヘッダへ残す。
    // BistroExteriorNight の1フレームで数えると **p50=5 / p90=9 / p99=13 / 最大18**
    // (8160タイル)。容量8では 11.9% のタイルが打ち切られる。
    // 【打ち切っても偏らない】載らなかった灯にも一様枝の確率が残るので、効率が落ちるだけ。
    // 4 / 8 / 12 / 16 を振っても |相対誤差| 中央値は 0.01442〜0.01467 で、
    // **同一構成2回のばらつきと同程度**しか動かなかった(= この場面では容量は効かない)。
    // 効かないなら小さいほうが安いので、p50を十分に覆う8にしてある
    inline constexpr int MegaLightsVisibleListCapacity = 8;
    // リスト枝へ回す割合 c。提案分布は
    //   q(y) = a/R + (1-a) * [ (1-c)*w_y/SumW + c*count_y/L ],  a = 0.25(固定)
    // で、**c をいくつにしても a は削らない**ため、届くどの灯にも正の下限確率が残る(=不偏)。
    //
    // 【0.5 にした根拠】c を 0 / 0.25 / 0.5 / 0.75 / 1.0 と振った実測(900枚蓄積・参照実装比):
    //   c      総和比    |相対誤差|中央値   暗黒点
    //   0.00   0.98490   0.01543          7.78%
    //   0.25   0.98511   0.01483          5.42%
    //   0.50   0.98560   0.01468          4.10%
    //   0.75   0.98655   0.01470          2.91%
    //   1.00   0.98615   0.01726            --
    // **総和比はどの c でも c=0 から +0.17% 以内**(不偏性の確認。許容は ±0.5%)。
    // 誤差の中央値は 0.5 が底で、**1.0 は c=0 より悪い** ―― 重み枝を殺すと
    // 距離減衰による重要度サンプリングを失うため。暗黒点だけなら 0.75 がさらに減るので、
    // 影の縁の黒点を最優先するなら 0.75 を選ぶ余地がある
    inline constexpr float MegaLightsVisibleListMix = 0.5f;
