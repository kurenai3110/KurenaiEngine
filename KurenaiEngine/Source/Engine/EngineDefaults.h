...
namespace Kurenai::Defaults
{
    ...
    // 【既定は無効】狙いは段階2で見つかった局所的な破綻を減らすことだったが、
    // 現状の実装では**入れると悪化する**。実測(RTX 4070 Ti / ManyLightsTest / 1280x720 /
    // N=256 / |相対誤差|の中央値):
    //   空間再利用なし        球 0.0309 / 平ら 0.0060 / 総和の相対差 -0.34%
    //   confidence重み        球 0.0729 / 平ら 0.0051 / 総和の相対差 +2.20%
    //   不偏化(Alg.6のZ)     球 0.0792 / 平ら 0.0051 / 総和の相対差 +4.68%
    // 平らな面では改善するが球で悪化し、しかも結合方式を「より正しい」形にするほど
    // バイアスが増える。**原因が特定できるまで既定では使わない。**
    // 起動オプション(-megalightsspatial 等)で切り替えて調べられるようにしてある
    ...
    // 空間再利用の結合でMIS重み(生成化バランスヒューリスティック)を使うか。
    // false は confidence(M)で重み付ける単純な結合で、近傍が自分と違う候補集合から
    // 引いている場合に不偏にならない。実測では総和の相対差が +2.2% 出た。
    // true は「その灯が隣のタイルへ届くか」を判定して定義域を厳密に扱うが、
    // 候補の数の2乗でG-Bufferを読み直すぶん重い
