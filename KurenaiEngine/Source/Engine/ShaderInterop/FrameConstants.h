...
// 自前の cbuffer を書くと、途中へフィールドを挿したときに**コンパイルは通るのに
// 絵だけが壊れる**(経緯は docs/ImplementationHistory.md 81章)。
//
// 【フィールドを足すときの手順】
...
namespace Kurenai::ShaderInterop
{
    ...
    struct alignas(16) FrameConstants
    {
        ...
        // 事前乗算されており(21.5節)、その倍率は時刻に連動して大きく動く。プリ露出済みの
        // 値をそのまま溜めると、DDGIの多重バウンスでズレが増幅され続ける。そこで書き込み時に
        // この倍率で割り、読み出し時に掛け直して、アトラスの中身を露出に依存しない物理量に保つ。
        // 踏んだときの実測は docs/ImplementationHistory.md 31.6.1。
