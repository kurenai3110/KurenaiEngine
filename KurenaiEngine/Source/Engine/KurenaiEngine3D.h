...
namespace Kurenai
{
    ...
    private:
        ...
        // FrameConstants.ShadowParams.wへ数値として渡し、3つのシェーダー(DirectLighting/
        // DeferredLighting/Transparent)が共有するSpecularEnergy.hlsliが方式を切り替える。
        // 値はSpecularEnergy.hlsliのKURENAI_SPEC_COMP_*と一致させること。
        //
        // 既定がLinearなのは、実使用域(エンジンはラフネスを[0.045, 1.0]にクランプする)では
        // 3方式のうち最も真値に近いことを多重散乱ランダムウォークとの比較で確認したため(14.9節)。
        ...
        SpecularCompensationMode m_SpecularCompensationMode = SpecularCompensationMode::Linear;
