...
namespace Kurenai
{
    ...
    private:
        ...
        // ワールド1mあたりのノイズ空間の距離(= 1/セルの広さ[m])。**これは基準の厚みでの値**で、
        // 実際にシェーダーへ渡すのも風のスクロールを進めるのも EffectiveCloudUvScale() の方。
        // 直接使うと厚みによる雲の大きさの変化(C7)が掛からない
        float m_CloudUvScale = Defaults::CloudUvScale;
        // 厚みで拡大したあとのUVスケール(C7)。
        // 【なぜ厚みで割るのか】厚みだけを上げると、幅が据え置きのまま縦に伸びた雲になる。
        // 実際の積雲は深い上昇流ほど水平方向のセルも大きく、厚みと幅は一緒に育つ。
        // そこで厚みに比例してセルを広げ、**厚みを「雲の大きさ」のつまみ**にする。
        // シーンのCellSizeの意味は「基準の厚みのときのセルの広さ[m]」になる。
        // 【シェーダーへ渡す値とスクロールを進める値は必ず同じであること】食い違うと、
        // CPUがkCloudNoisePeriodで巻き戻した位置とシェーダー側の周期境界がずれ、
        // 風が吹くたびに雲がジャンプする(m_CloudScrollOffsetのコメント参照)
        float EffectiveCloudUvScale() const;
