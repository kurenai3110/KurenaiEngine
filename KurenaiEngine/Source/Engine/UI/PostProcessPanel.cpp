...
namespace Kurenai::UI
{
    ...
    {
        using TonemapCurve = Kurenai::TonemapCurve;

        BeginParamGroup();

        static const char* kTonemapCurveNames[] = { "Reinhard", "ACES", "AgX" };
        int curveIndex = static_cast<int>(m_Engine.m_PostProcessSettings.Curve);
        if (ComboEx(
        ...
        {
            m_Engine.m_PostProcessSettings.Curve = static_cast<TonemapCurve>(curveIndex);
        }

        SliderFloatEx(
            "薄明視###MesopicVision", &m_Engine.m_PostProcessSettings.MesopicStrength, 0.0f, 1.0f, Defaults::MesopicStrength, "%.2f", 0,
        ...
        CheckboxEx(
            "出力ディザリング###OutputDithering", &m_Engine.m_PostProcessSettings.DitherEnabled, Defaults::DitherEnabled,
    ...
    void PostProcessPanel::DrawTAASection()
    {
        ...
        if (CheckboxEx(
                "TAAを有効にする###EnableTAA", &m_Engine.m_PostProcessSettings.TAAEnabled, Defaults::TAAEnabled,
        ...
        {
            SliderFloatEx(
                "履歴ブレンド率###TAABlendWeight", &m_Engine.m_PostProcessSettings.TAABlendWeight, 0.02f, 0.5f, Defaults::TAABlendWeight,
                "%.3f", 0,
                "今フレームの色を混ぜる割合。小さいほど多くのフレームが平均されて滑らかになるが、"
                "遮蔽が変わったときの追従が遅くなり残像が出やすくなる。"
                "フレームレートに対して固定の割合なので、fpsが変わると残像の長さも変わる");
            SliderFloatEx(
                "ジッター強度###TAAJitterScale", &m_Engine.m_PostProcessSettings.TAAJitterScale, 0.0f, 1.5f, Defaults::TAAJitterScale,
                "%.2f", 0,
                "サンプル位置を散らす幅の倍率(1.0でピクセル内いっぱい)。0にすると時間方向の"
                "スーパーサンプリング効果だけが消え、再投影と蓄積によるノイズ低減は残る");
            SliderFloatEx(
                "シャープネス###TAASharpness", &m_Engine.m_PostProcessSettings.TAASharpness, 0.0f, 1.0f, Defaults::TAASharpness, "%.2f", 0,
                "蓄積で失われる高域を戻す量。上げすぎると輪郭に白いふちが出る。"
                "トーンマップ後の最終出力にのみ掛かるため、上げてもちらつきは増えない");

            SliderFloatEx(
                "静止時のちらつき抑制###TAAAntiFlicker", &m_Engine.m_PostProcessSettings.TAAAntiFlicker, 0.0f, 1.0f,
        ...
        using TAAClipMode = Kurenai::TAAClipMode;
            ...
            int clipModeIndex = static_cast<int>(m_Engine.m_PostProcessSettings.TAAClip);
            if (ComboEx(
            ...
            {
                m_Engine.m_PostProcessSettings.TAAClip = static_cast<TAAClipMode>(clipModeIndex);
            }

            if (m_Engine.m_PostProcessSettings.TAAClip != TAAClipMode::None)
            {
                SliderFloatEx(
                    "履歴の許容幅###TAAClipGamma", &m_Engine.m_PostProcessSettings.TAAClipGamma, 0.5f, 3.0f, Defaults::TAAClipGamma,
                    "%.2f", 0,
    ...
    void PostProcessPanel::DrawBloomSection()
    {
        ...
        CheckboxEx(
            "ブルームを有効にする###EnableBloom", &m_Engine.m_PostProcessSettings.BloomEnabled, Defaults::BloomEnabled,
            "明るい部分の光がにじむレンズ散乱を再現する");

        if (m_Engine.m_PostProcessSettings.BloomEnabled)
        {
            SliderFloatEx(
                "強さ###BloomStrength", &m_Engine.m_PostProcessSettings.BloomStrength, 0.0f, 0.5f, Defaults::BloomStrength, "%.3f", 0,
                "元の映像へブルームを合成する比率");
            SliderFloatEx(
                "しきい値###BloomThreshold", &m_Engine.m_PostProcessSettings.BloomThreshold, 0.0f, 8.0f, Defaults::BloomThreshold, "%.2f",
                0,
                "この輝度を超えた分だけをブルームの元にする。物理的にはレンズ散乱なので全輝度に"
                "かかるのが正しく、既定は低めにしてある");
            SliderFloatEx(
                "ソフトニー###BloomSoftKnee", &m_Engine.m_PostProcessSettings.BloomSoftKnee, 0.0f, 1.0f, Defaults::BloomSoftKnee, "%.2f",
    ...
    void PostProcessPanel::DrawAutoExposureSection()
    {
        ...
        CheckboxEx(
            "自動露出を有効にする###EnableAutoExposure", &m_Engine.m_PostProcessSettings.AutoExposureEnabled, Defaults::AutoExposureEnabled,
        ...
        {
            SliderFloatEx(
                "EV100 下限###AEMinEV100", &m_Engine.m_PostProcessSettings.AutoExposureMinEV100, -8.0f, 20.0f,
                Defaults::AutoExposureMinEV100, "%.1f", 0, "自動露出が選べるEV100の下限(暗い側の限界)");
            SliderFloatEx(
                "EV100 上限###AEMaxEV100", &m_Engine.m_PostProcessSettings.AutoExposureMaxEV100, -8.0f, 20.0f,
                Defaults::AutoExposureMaxEV100, "%.1f", 0, "自動露出が選べるEV100の上限(明るい側の限界)");
            SliderFloatEx(
                "露出補正###AECompensation", &m_Engine.m_PostProcessSettings.AutoExposureCompensation, -4.0f, 4.0f,
                Defaults::AutoExposureCompensation, "%.2f EV", 0,
                "自動で決まった露出へ加えるオフセット。写真の露出補正と同じ意味で、"
                "+1で1段(2倍)明るくなる");
            SliderFloatEx(
                "測光上限(基準EVから)###AEKeyCeiling", &m_Engine.m_PostProcessSettings.AutoExposureKeyCeilingEV, -4.0f, 16.0f,
            ...
            SliderFloatEx(
                "夜のロールオフ###AENightRolloff", &m_Engine.m_PostProcessSettings.AutoExposureNightRolloffEV, 0.0f, 8.0f,
            ...
            SliderFloatEx(
                "ロールオフ 暗側の折れ点###AENightRolloffDark", &m_Engine.m_PostProcessSettings.AutoExposureNightRolloffDarkEV100, -8.0f,
                8.0f, Defaults::AutoExposureNightRolloffDarkEV100, "%.1f", 0,
                "測定EV100がこれ以下なら夜のロールオフが最大量かかる。既定の-2は満月の夜の地表のすぐ上");
            SliderFloatEx(
                "ロールオフ 明側の折れ点###AENightRolloffBright", &m_Engine.m_PostProcessSettings.AutoExposureNightRolloffBrightEV100,
                -8.0f, 20.0f, Defaults::AutoExposureNightRolloffBrightEV100, "%.1f", 0,
                "測定EV100がこれ以上なら夜のロールオフは掛からない。既定の10は曇天の屋外あたり");
            SliderFloatEx(
                "明順応の速さ###AESpeedUp", &m_Engine.m_PostProcessSettings.AutoExposureSpeedUp, 0.1f, 10.0f, Defaults::AutoExposureSpeedUp,
                "%.2f", 0, "暗い場所から明るい場所へ移ったときに露出が追従する速さ");
            SliderFloatEx(
                "暗順応の速さ###AESpeedDown", &m_Engine.m_PostProcessSettings.AutoExposureSpeedDown, 0.1f, 10.0f,
                Defaults::AutoExposureSpeedDown, "%.2f", 0,
                "明るい場所から暗い場所へ移ったときに露出が追従する速さ。実際の目と同様、"
                "明順応より遅くするのが自然");
            SliderFloatEx(
                "測光 下側除外率###AELowPercentile", &m_Engine.m_PostProcessSettings.AutoExposureLowPercentile, 0.0f, 1.0f,
                Defaults::AutoExposureLowPercentile, "%.2f", 0,
                "輝度ヒストグラムの下側何割を露出の平均から除外するか。暗すぎる画素に露出が"
                "引きずられるのを防ぐ");
            SliderFloatEx(
                "測光 上側除外位置###AEHighPercentile", &m_Engine.m_PostProcessSettings.AutoExposureHighPercentile, 0.0f, 1.0f,
